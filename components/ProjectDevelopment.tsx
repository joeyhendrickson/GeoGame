...
export default function ProjectDevelopment() {
  ...
  const [allSectionsFilled, setAllSectionsFilled] = useState(false);

  useEffect(() => {
    setAllSectionsFilled(sections.length > 0 && sections.every(s => !!s.filledContent));
  }, [sections]);
  ...
  const cleanContent = (content: string): string => {
    ...
    // Remove "AI generated content" or similar phrases
    cleaned = cleaned.replace(/AI[- ]generated content/gi, '');
    cleaned = cleaned.replace(/AI[- ]generated/gi, '');
    cleaned = cleaned.replace(/generated by AI/gi, '');
    cleaned = cleaned.replace(/This content was generated/gi, '');
    return cleaned.trim();
  };
  ...
  const handleFillSection = async (sectionId: string) => {
    ...
    try {
      ...
      if (data.success) {
        const cleanedContent = cleanContent(data.filledContent);
        setSections(sections.map(s =>
          s.id === sectionId
            ? { ...s, filledContent: cleanedContent }
            : s
        ));
      } else {
  ...
  const handleFillAll = async () => {
    ...
    // Close all expanded sections and disable expansion during filling
    setExpandedSections(new Set());
    ...
    try {
      ...
      for (let i = 0; i < unfilledSections.length; i++) {
        const section = unfilledSections[i];
        
        // Fill section directly to ensure state updates properly
        setIsFilling(prev => ({ ...prev, [section.id]: true }));
        ...
        try {
          ...
          const response = await fetch('/api/document/fill-section', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
          ...
          setIsFilling(prev => ({ ...prev, [section.id]: false }));
        }
      ...
      setError(error instanceof Error ? error.message : 'Failed to fill all sections');
    } finally {
        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <button
              ...
              disabled={isFillingAll || allSectionsFilled}
            ...
            >
              {isFillingAll ? 'Filling All...' : 'Fill All Sections'}
            </button>
          </div>
                ...
                <div key={section.id} className="bg-white border-2 border-gray-200 rounded-xl shadow-sm overflow-hidden">
                  ...
                  <div className="flex items-start justify-between p-6">
                    <button
                      type="button"
                      onClick={() => {
                        // Only allow expanding/collapsing if all sections are filled or if not in fill-all process
                        if (allSectionsFilled || !isFillingAll) {
                          toggleSection(section.id);
                        }
                      }}
                      disabled={isFillingAll && !allSectionsFilled}
                      className="flex items-start gap-3 flex-1 text-left bg-transparent border-0 cursor-pointer hover:bg-gray-50 -m-6 p-6 w-full transition-colors select-none disabled:opacity-50 disabled:cursor-not-allowed"
                      title={isFillingAll && !allSectionsFilled ? 'Sections cannot be viewed while filling all sections' : 'Toggle section view'}
                    >
                  ...
                  {/* Section Content - Expandable - Only show if expanded OR all sections are filled */}
                  {(isExpanded || allSectionsFilled) && (
          ...
          <div className="flex justify-center pt-4">
            <button
              ...
              disabled={isGenerating || !allSectionsFilled}
              className="px-8 py-3 bg-gradient-to-r from-black to-black text-white rounded-xl font-semibold shadow-lg hover:shadow-xl hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200"
            >
          ...
          <div className="flex items-start justify-between mb-6">
            ...
            <div className="flex gap-3">
              <button
                ...
                className="px-6 py-3 text-white border-2 border-white rounded-xl font-semibold hover:bg-white hover:text-black transition-all duration-200 flex items-center gap-2"
              >
              ...
              <button
                ...
                className="px-6 py-3 bg-white text-black rounded-xl font-semibold shadow-lg hover:shadow-xl hover:scale-105 transition-all duration-200 flex items-center gap-2"
              >
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
